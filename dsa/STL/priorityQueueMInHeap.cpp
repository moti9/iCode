...
# Def :: In the case of the min heap, priority queues are a type of container adaptors, specifically designed such that its first element is always the smallest of the elements it contains and the rest elements are in increasing order.

# The syntax for a min-heap priority queue :: priority_queue<object_type,vector<object_type>,greater<object_type>> variable_name;
...
template <typename T>
void printpriorityqueue(priority_queue<T, vector<T>, greater<T>> pq)
{
    while (!pq.empty())
    {
        cout << pq.top() << " ";
