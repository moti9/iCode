...
# Def :: unordered_map in STL are associative containers where each element consists of a key value and a mapped value. Two mapped values cannot have the same key value. The elements can be in any order.
...
void printMap(unordered_map<T1, T2> &mp)
{
    cout << "Elements present in the unordered map : " << endl;
